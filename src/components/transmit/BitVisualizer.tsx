...
export function BitVisualizer({ bits }: BitVisualizerProps): React.JSX.Element {
  return (
    <section className="rounded-lg border border-emerald-500/40 bg-zinc-900/80 p-4">
      ...
      <div className="grid min-h-16 grid-cols-8 gap-1 md:grid-cols-12">
        {bits.length === 0 ? (
          <p className="col-span-8 text-xs text-zinc-500 md:col-span-12">No signal playing</p>
        ) : (
          bits.map((bit: number, index: number) => (
            <div
              key={`${index}-${bit}`}
              className={`h-6 rounded border text-center text-xs leading-6 ${
                bit === 1
